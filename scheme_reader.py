...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 7
        "*** YOUR CODE HERE ***"
        # quotes is in first element of pair and rest is the expression
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # if next token is a closing parenthesis return the expression
            # otherwise raise a SyntaxError with error message
            if src.current() == ')':
                src.remove_front()
                return expr
            else:
                raise SyntaxError('Incorrect syntax for {0}'.format(src))
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
